...
const RELEASES = [
  {
    ...
    abilities: [
      ...
      { icon: '📥', name: 'Multi-Source Context Ingestion', desc: 'Add Project supports GitHub, GitLab (self-hosted included), and BASE_CODE_DIR-relative directory intake.' },
      { icon: '🛡️', name: 'Hardened Runtime + Security', desc: 'Container hardening, token-safe errors, directory traversal guards, and API-only server deployment path.' },
      { icon: '📋', name: 'MCP/System Observability', desc: 'MCP guide logs, server env/directories visibility, source config diagnostics, and clearer runtime errors.' },
      { icon: '🧭', name: 'Client-Local MCP Agent Config', desc: 'AI Agent MCP status/setup now checks local desktop config files via Electron bridge with fallback API paths.' },
    ],
    items: [
      { type: 'refactor', text: 'Completed phase-1 split: savant-app client and savant-server backend boundaries enforced across runtime, APIs, and docs' },
      { type: 'feat', text: 'Client local session service now owns provider session CRUD/metadata and emits live refresh events to renderer' },
      { type: 'feat', text: 'Workspace-session link contract added so server tracks cross-provider workspace/session associations cleanly' },
      { type: 'feat', text: 'Context source-gated ingestion API delivered: /api/context/repos/sources + source payload add flow (github|gitlab|directory)' },
      { type: 'feat', text: 'Ingestion engine supports canonical repo naming, existing checkout update-in-place, branch checkout/pull, and self-hosted GitLab detection' },
      { type: 'feat', text: 'Directory ingestion enforces BASE_CODE_DIR normalization and blocks traversal/out-of-base paths with readable errors' },
      { type: 'feat', text: 'Context overview now displays project source metadata (provider/origin/server path) and keeps add flow visible in empty states' },
      { type: 'feat', text: 'Abilities seed bundle expanded (personas/rules/policies) with bootstrap endpoint and empty-store bootstrap controls in UI' },
      { type: 'feat', text: 'MCP Tool Guide now includes recent action logs and improved restart/test/setup visibility for runtime debugging' },
      { type: 'feat', text: 'System Info expanded with server directories/env visibility and explicit no-project-source server-side guidance message' },
      { type: 'feat', text: 'Left action rail gained UI-only hard refresh control and improved terminal/UI mode behavior' },
      { type: 'feat', text: 'One-command Docker Compose server runtime added with BASE_CODE_DIR host mount and hardened container defaults' },
      { type: 'fix', text: 'Resolved session_service module/runtime packaging failures in client startup and test harness flows' },
      { type: 'fix', text: 'Resolved AI Agent MCP Config false negatives by moving status/setup checks to client-local filesystem bridge' },
      { type: 'fix', text: 'Improved navigation/load failure handling (file hash routing, blank-page diagnostics) and surfaced errors in client logs' },
      { type: 'test', text: 'Expanded test matrix: server ingestion/bootstrap tests, client modal mapping tests, Electron smoke/mode tests, MCP config Playwright tests, and monorepo integration checks' },
      { type: 'docs', text: 'Updated README, AGENTIC/AGENTS, in-app Savant Guide, PRDs, and memory_bank architecture docs to match branch implementation' },
    ],
  },
  {
    version: 'v8.0.0',
    date: '2026-04-21',
    tag: 'major',
    tagline: 'Client-Server Rewrite hero release. Savant is now split into savant-app + savant-server.',
    description: 'SAVANT v8.0.0 is the client-server rewrite baseline. The Electron client now owns UI rendering, xterm/terminal surfaces, D3 visualizations, local cache, and offline mutation queue behavior. The server now owns centralized API/MCP orchestration, shared persistence, and knowledge graph data. This release formalizes deployment boundaries for enterprise installs where server infrastructure is centrally managed and desktop clients connect through stable API contracts.',
    abilities: [
      { icon: '🖥️', name: 'Client Ownership', desc: 'UI runtime, terminal/xterm, D3 views, and local agent interactions are fully client-side.' },
      { icon: '🌐', name: 'API Contract', desc: 'Client-server communication is standardized over HTTP/SSE with explicit integration boundaries.' },
      { icon: '🗄️', name: 'Centralized Server Data', desc: 'Server becomes source of truth for shared data, including knowledge graph and API persistence.' },
      { icon: '📦', name: 'Mountable Storage', desc: 'Server data root supports external mount paths for Docker/Kubernetes deployments.' },
      { icon: '📡', name: 'Offline Queue Replay', desc: 'Client queues server mutations while offline and replays in FIFO order when connectivity returns.' },
    ],
    items: [
      { type: 'refactor', text: 'Split monolith runtime into savant-app (client) and savant-server (backend)' },
      { type: 'refactor', text: 'Moved UI rendering ownership to client: index/detail pages, terminal surfaces, and visualization modules' },
      { type: 'refactor', text: 'Server responsibilities narrowed to API + MCP orchestration + centralized data persistence' },
      { type: 'feat', text: 'Introduced server data root strategy for mountable DB and abilities storage in containerized deployments' },
      { type: 'feat', text: 'Added abilities seed bootstrap so new server installs can initialize required assets automatically' },
      { type: 'feat', text: 'Server startup now creates DB when absent and initializes schema/migrations deterministically' },
    ],
  },
  {
    version: 'v7.2.0',
  ...
  },
  {
    ...
    date: '2026-04-12',
    tag: 'major',
    tagline: 'Gemini and Codex are now first-class providers. Session capture is tougher. UI flows are complete.',
    description: 'SAVANT v6.5.0 is the Gemini and Codex hero release. Gemini session support is now complete end-to-end, Codex session plumbing is hardened alongside it, and the shared dashboard flows are more reliable under real CLI usage. Savant now captures full chat streams with stronger fallback parsing for in-progress sessions and artifact directories while tightening MCP startup and provider metadata handling across both providers.',
    abilities: [
      ...
      { icon: '🧠', name: 'Codex Provider Hardening', desc: 'Codex session plumbing, provider metadata handling, and shared dashboard integration are more resilient under real CLI usage.' },
      { icon: '🗂', name: 'Workspace-Ready Shared UI', desc: 'Assign, rename, star, archive, search, inspect, and delete Gemini and Codex sessions from the same unified workflow.' },
      { icon: '💬', name: 'Stored Provider Chat History', desc: 'Conversation parsing keeps prompts, replies, thoughts, and tool calls together for richer Gemini and Codex session detail.' },
    ...
    items: [
      ...
      { type: 'feat', text: 'Codex and Gemini now share the same hardened provider plumbing so session capture and dashboard actions behave consistently' },
      ...
      { type: 'feat', text: 'Gemini and Codex providers support the shared UI actions for workspace management, search, inspection, and project context' },
      { type: 'feat', text: 'Provider conversation parsing now preserves prompts, replies, thoughts, and tool call metadata for richer detail pages' },
      { type: 'fix', text: 'Gemini file browser now resolves the real artifact directory instead of assuming a flat session path' },
      { type: 'fix', text: 'Workspace assignment and provider metadata updates now validate Codex and Gemini sessions before mutating cache or notes' },
      ...
      { type: 'chore', text: 'Package version bumped to 6.5.0 with hero release notes and expanded Gemini/Codex regression coverage' },
    ],
  },
  {
    version: 'v6.4.0',
...
function getDetailPageUrl(id) {
  const mode = currentMode || 'copilot';
  return `./detail.html?mode=${encodeURIComponent(mode)}&session_id=${encodeURIComponent(id)}`;
}
